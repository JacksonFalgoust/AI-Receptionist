...
remain **console-only**: Publish updates the live guide's *instructions* and
nothing else, so console knowledge edits do not change what the guide can
look up on a call (see "Publishing console configuration to the guide"
below for why, and "Known gaps"). The guide's knowledge base is edited in
the GuideAnts editor.
...
cancel and restart an in-flight reply (`app/barge_in.py`), or whether an
unrecognized WS message is one of Twilio's speaker events
(`app/speaker_events.py`).

```
                    POST /twiml (TwiML)            WS /ws (JSON frames)
Caller ⇄ Twilio ⇄ ───────────────────── ⇄ app/main.py ⇄ ───────────────────── ⇄ Twilio Conversation Relay
                                            │
                                            │ HTTPS, OpenAI SDK
                                            ▼
...
| File | Role |
|---|---|
| [app/config.py](../app/config.py) | Reads all settings from environment / `.env`, no other file touches `os.environ`. |
| [app/guide_client.py](../app/guide_client.py) | Thin wrapper around the `openai` SDK pointed at GuideAnts' Responses API; exposes `stream_reply` (one turn) and `build_input` (interruption-note folding), plus the `GuideSession` continuation handle. |
| [app/fillers.py](../app/fillers.py) | Pure functions used by `app/main.py` to decide the filler-phrase behavior: `looks_like_question` classifies a caller utterance as question/request-like (warrants a filler) or not, `pick` returns a random filler phrase from a list, and `is_backchannel` classifies an utterance as pure acknowledgment noise (e.g. "ok", "yeah") that should never get a guide reply. No I/O, no Twilio/GuideAnts knowledge. |
| [app/barge_in.py](../app/barge_in.py) | Pure functions used by `app/main.py` to decide selective barge-in: `is_stop_command` matches a caller utterance against a built-in stop/wait phrase list, and `should_interrupt` (= `is_stop_command` or `fillers.looks_like_question`) decides whether an utterance heard mid-reply should cancel the in-flight reply at all. No I/O, no Twilio/GuideAnts knowledge — unit-tested directly in `tests/test_barge_in.py`. |
| [app/speech_timing.py](../app/speech_timing.py) | One pure function, `estimate_seconds`, used by `app/main.py` to estimate how long Twilio's TTS will take to speak a given text from its word count. Fallback pacing until the first agent-stopped speaker event is recognized on a call, and the basis of the ceiling on waiting for one. No I/O. |
| [app/speaker_events.py](../app/speaker_events.py) | One pure function, `classify`, used by `app/main.py` to recognize Twilio's speaker-event WS messages (`agentSpeaking`/`clientSpeaking` start/stop, subscribed via `events="speaker-events"` in the TwiML). Matches loosely since Twilio doesn't document the exact JSON shape — unit-tested in `tests/test_speaker_events.py`. No I/O. |
| [app/main.py](../app/main.py) | FastAPI app with the two endpoints Twilio talks to: `POST /twiml` and `WS /ws`. All call-handling logic lives here; also mounts `reservations_api.router`. |
...
| [app/guide_publish/guide_dto.py](../app/guide_publish/guide_dto.py) | Pure, no I/O: `build_update_dto()` rebuilds GuideAnts' full-state `UpdateGuideDto` from a `GET /api/guides/{id}` body with only the instructions changed; `unsupported_features()` is the fail-closed guard listing anything on the guide this round-trip has not been verified to preserve; `comparable()` is the order-insensitive normalized view used to prove, after the write, that nothing else moved. |
| [app/guide_publish/guideants_admin.py](../app/guide_publish/guideants_admin.py) | The only module that calls GuideAnts' *authoring* API, using `GUIDEANTS_ADMIN_EMAIL`/`GUIDEANTS_ADMIN_PASSWORD` (a GuideAnts Admin-role user) — a different credential from the published guide's `GUIDEANTS_API_KEY` used on calls. `update_guide_instructions()` does a verified read-modify-write: `POST /api/auth/login`, `GET /api/guides` (match by `GUIDEANTS_GUIDE_NAME`), `GET /api/guides/{id}`, refuse on `unsupported_features`, `PUT /api/guides/{id}`, then `GET` again and compare — a failed verification attempts one restore and raises. Holds the login cookie, re-authenticating once on a 401. It has **no import path**: `import_bundle` was deleted, not deprecated. |
| [app/guide_publish/publisher.py](../app/guide_publish/publisher.py) | Orchestrates a publish: render -> bundle (locally, for the invariants and the hash) -> push the rendered *instructions* -> record. `latest_succeeded()` is what `app/main.py`'s greeting reads; `republish()` re-sends a stored publication's `instructions_text` as the rollback path. Adds `KNOWLEDGE_NOT_SYNCED_WARNING` to every publication of a guide that has knowledge items. |
...
|---|---|---|---|
| `GUIDEANTS_BASE_URL` | `GUIDEANTS_BASE_URL` | `http://localhost:5107` | `app/guide_client.py` — base URL of the GuideAnts backend |
| `GUIDEANTS_PUB_ID` | `GUIDEANTS_PUB_ID` | `""` | `app/guide_client.py` — published guide GUID, part of the URL path |
| `GUIDEANTS_API_KEY` | `GUIDEANTS_API_KEY` | `"anonymous"` | `app/guide_client.py` — sent as `Authorization: Bearer <key>` |
...
| `TURN_PAUSE_SECONDS` | `TURN_PAUSE_SECONDS` | `0.5` | `app/main.py` (`schedule_turn()`) — how long the caller must stay quiet after a transcribed prompt before the buffered text is committed as their turn; a clientSpeaking-start speaker event during the wait holds the buffer open for the continuation instead (see "Turn-pause buffering" below). The only latency this feature adds to an ordinary turn. |
| `TURN_RESUME_GRACE_SECONDS` | `TURN_RESUME_GRACE_SECONDS` | `1.5` | `app/main.py` — after the caller resumes speaking mid-buffer and then stops again, how long to wait for the continuation's transcript (which trails the clientSpeaking-stop event by STT finalization time) before giving up and committing the buffered text alone. |
| `EXTRA_BACKCHANNEL_PHRASES` | `EXTRA_BACKCHANNEL_PHRASES` | `[]` (empty) | `app/main.py` (via `fillers.is_backchannel`) — comma-separated phrases, on top of the built-in `fillers.BACKCHANNEL_PHRASES`, that count as pure acknowledgment noise and should never get a guide reply. |
...
`get_caller_phone_number` needed this from the start: GuideAnts' outgoing
tool-call HTTP requests (`ToolCaller.ExecuteWebApiAsync` in the GuideAnts
repo) carry only static, guide-configured auth (header/query/OAuth) plus
whatever arguments the model fills in — no conversation, call, or session
identifier is ever attached. An HTTP endpoint like `/api/reservations/caller`
would therefore have no way to know *which* call is asking, and would answer
identically (or fail) for every concurrent call.

The reservation operations used to be a **Web API** tool source instead —
GuideAnts called `/api/reservations/*` on this app directly, authenticated
with a shared `X-Api-Key`/`RECEPTIONIST_API_KEY` header. That meant a working
copy of that header was enough to check availability or create/cancel a
reservation from anywhere, not just from an active call through this app.
Moving them to Client Actions closes that off: GuideAnts never dials out for
them at all, there's no HTTP endpoint or shared secret to leak or replay, and
`app/reservations.py`/`app/payments.py`'s Booqable business logic is now only
reachable through `app/guide_client.py`'s in-process tool dispatch, mid-call.

**Declared in GuideAnts, not in this app's code.** Each tool's schema
(`guide-demo/caller-phone-client-tool.json`,
`guide-demo/reservations-client-tool.json`) is pasted into the guide's
**Tools** tab as its own **Client Actions** tool source (see SETUP.md step
1.7); both use a `servers[0].url` with a `client://` scheme instead of
`https://`. GuideAnts classifies any tool source by that URL scheme
...
- `app/main.py`'s `setup` handler sets `st.guide.caller_phone = msg.get("from")`
  ...
  of that value, read by the `get_caller_phone_number` handler.
- This app never attaches a `tools` kwarg to any `responses.create` call —
  GuideAnts already knows the tools exist from the guide's own persisted
  config, so they're included on every turn automatically.
- When the model emits a `function_call` output item, this app never lets that
  ...
  - `_stream_reply_with_tools` is the loop that actually drives one caller
    ...
    immediately issues a follow-up `responses.create` whose `input` is the
    list of `{"type": "function_call_output", "call_id": ..., "output": ...}`
    results, still passing the same `conversation` id. This repeats — a
    single turn can involve more than one tool round-trip — capped at
...
- **If you edit `caller-phone-client-tool.json` or
  `reservations-client-tool.json` later**, re-paste the edited file into its
  own Client Actions Schema tab and save — the file on disk isn't read live
  and there's no auto-sync (see SETUP.md step 1.7).
...
- Builds and returns TwiML:
  ...
  ```
- `interruptible="none"` + `reportInputDuringAgentSpeech="speech"` together mean Twilio itself never pauses or stops TTS playback because of caller speech, but caller speech heard during agent speech is still transcribed and delivered to this app as a `prompt` message (instead of the `interrupt` message Twilio would send under `interruptible="speech"`). What this app does with that reported speech — ignore it, or cancel and restart the reply for it — is a decision made in `app/main.py`/`app/fillers.py`/`app/barge_in.py`; see "Filler phrases and mid-reply speech" and "Selective barge-in" below.
...
- `events="speaker-events"` subscribes to Twilio's notifications that the agent (TTS playback) or client (caller) started/stopped speaking. The agent-stopped event is the real "the reply finished playing" signal used to hold the reply task open (see "`start_reply()` / `respond_to()`" below); Twilio documents the subscription attribute but not the messages' JSON shape, so `app/speaker_events.py` recognizes them loosely and the app falls back to the word-count estimate until the first agent-stop is recognized on a call. The shape actually observed on live calls is `{"type": "info", "name": "agentSpeaking"|"clientSpeaking", "value": "on"|"off"}` — pinned in `tests/test_speaker_events.py`.
...
- `guide_awaiting_answer: bool` — `True` once the guide's most recently completed reply ended in a `?`, i.e. it's waiting on the caller to answer. Set by `respond_to()` alongside logging the reply to `st.messages`; reset to `False` at the start of every new turn in `start_reply()`, so a barge-in that cancels a reply before it completes can't leave a stale flag from an earlier, unrelated turn in place. Checked by the `prompt` handler ahead of `fillers.is_backchannel()` (see "Late backchannel" below) so a short affirmative answering a pending question isn't swallowed as noise.
...
| `prompt` | `voicePrompt` (caller's transcribed speech) | If a reply *is* already streaming: `barge_in.should_interrupt()` checks whether this is a stop/wait phrase or a new question. If so, the in-flight reply is cancelled; a stop/wait phrase then just gets a local acknowledgment and silence, a new question starts a fresh reply for this utterance (see "Selective barge-in" below). Otherwise the text is just logged — not spoken, not acted on, not recorded anywhere. If no reply is streaming: text merging into an already-buffered turn skips further classification (a mid-turn fragment is a continuation, not noise); otherwise, if `st.guide_awaiting_answer` is set (the guide's last reply ended in a `?`), the utterance is scheduled as the caller's answer without running the backchannel check at all; otherwise `fillers.is_backchannel()` catches pure acknowledgment noise (e.g. "ok", "yeah") and it's just logged, never recorded into `messages` either — this also covers the case where STT finishes transcribing a short "ok" just *after* the reply already finished. Anything else is buffered by `schedule_turn()` and committed as the caller's turn only after `TURN_PAUSE_SECONDS` of further silence (see "Turn-pause buffering" below); the commit calls `start_reply()`, which also decides via `fillers.looks_like_question()` whether to prepend a filler phrase. |
...
3. **Late backchannel, after the reply already finished.** Twilio's speech-to-text can finish transcribing a short utterance like "ok" slightly *after* the reply (and its playback-end hold — the agent-stopped speaker event or the estimate fallback, see above) has actually finished, so by the time the `prompt` arrives `st.task.done()` is already `True` — it no longer looks like "mid-reply" at all. Checked first: if `st.guide_awaiting_answer` is set — the guide's last completed reply ended in a `?` — the utterance is scheduled as the caller's answer to that question via `schedule_turn()`, skipping the backchannel check entirely; to `fillers.is_backchannel()` a caller answering "yeah" to a pending question looks identical to the late-STT trailing acknowledgment below, so this check has to come first. Otherwise, `fillers.is_backchannel(text, config.EXTRA_BACKCHANNEL_PHRASES)` checks whether the whole utterance is pure acknowledgment noise (`fillers.BACKCHANNEL_PHRASES`: "ok", "okay", "yeah", "mmhmm", "got it", ... plus anything in the env-configurable extra list). If so, it's just logged ("Ignored backchannel utterance (not acted on)") — never sent to GuideAnts, never logged to `st.messages`: it's genuinely noise, not part of the conversation.
4. **Starting a real reply.** Otherwise, `start_reply(text)` runs: it logs the caller's text to `st.messages` and checks `fillers.looks_like_question(text)` — true for text ending in `?`, or starting (after stripping leading fillers like "um"/"okay"/"so") with an interrogative word (`what`, `how`, `can`, `is`, ...) or a request verb (`tell`, `find`, `help`, ...), or a lead-in phrase like "i need"/"i want"/"looking for". This result (`filler_eligible`) only decides whether a filler is a *candidate* for *any* wait during this turn, first or later — a filler is only actually spoken for a given wait if GuideAnts also hasn't produced the next delta within `config.FILLER_DELAY_SECONDS` (see "`start_reply()` / `respond_to()`" above). So a filler-eligible question that GuideAnts answers quickly gets no filler, a non-eligible utterance (e.g. "thank you") never gets one no matter how slow GuideAnts is, and a filler-eligible question with multiple slow tool-call rounds may get more than one filler phrase spoken in sequence, one per slow wait.
...
playback on its own. Instead, `app/main.py` decides, per mid-reply `prompt`,
...
4. **The inverse case produces no note at all.** If a barge-in lands during
   ...
   no way for `app/main.py` to know the caller missed the tail end — the guide
...
manual connectivity check). None of this touches the Twilio WS path directly
either — no streaming, no per-call state — it's plain async function calls
made from inside `_execute_tool`/`_run_reservation_tool` while a tool-call
round-trip is in flight for some call's turn.

```
GuideAnts (function_call, unresolved) ⇄ this app (app/guide_client.py) ⇄ Booqable JSON:API v4
```
...
- **`app/reservations.py`** — the actual find/check/book/reserve/register logic
  ...
  `list_customers`, `add_customer_note`, `register_customer`, `get_order_contact`,
  plus small helpers for resolving the account's one active location and
...
### `createReservation` argument shape

The arguments are flat `customer_name` (required), `customer_email`, and
`customer_phone` fields — **not** a nested `customer: {name, email, phone}`
object, even though that would be the more natural shape. This is a
deliberate workaround, not a style choice: GuideAnts' OpenAPI-to-tool-schema
converter (`OpenApiHelper.cs`, `GetToolDefinitionsFromSchema`, in the
GuideAnts repo) only reads top-level `requestBody` `properties`/`required` —
it has no handling for a nested object property's own `properties`/`required`
...
own test suite) and are what the guide can actually see and fill in — see
`guide-demo/reservations-client-tool.json`'s `createReservation` schema.

`_run_reservation_tool` requires at least one of `customer_email`/
`customer_phone` for `createReservation` (raising `BooqableError`, turned
into an `{"error": ...}` tool result, if neither is given) — this is the
real enforcement; the schema's property descriptions are only a hint to the
model, not something GuideAnts validates before handing the call back.
...
### `listCustomers`/`createCustomer` — standalone customer operations
...
- `listCustomers` returns `reservations.list_customers`'s mapped list
  (`customer_id`/`name`/`email`/`phone`). The guide's instructions tell it to
  use this only to look someone up, never to read the whole list back to a
  caller — it's plain unfiltered data, gated only by the tool being
  client-handled (only reachable mid-call, through this app).
- `createCustomer` calls `reservations.register_customer`, which just wraps
  the same `find_or_create_customer` used by `create_reservation` — so it's
  equally idempotent (dedupes by email/phone, never creates a duplicate) and
  shares the same flat-argument reasoning as `createReservation` above (a
  nested `customer` object wouldn't reach the LLM's tool definition). It
  additionally accepts an optional `note`: if given, `register_customer`
  calls `add_customer_note` to attach it to the customer via Booqable's
  **Notes** resource — `POST /notes` with
...
still exist) is unauthenticated (it only reports whether the key is
configured and the account's timezone), same trust level as the
already-unauthenticated `/twiml`.

**Config**: `app/config.py` reads `BOOQABLE_COMPANY_URL`, `BOOQABLE_API_KEY`,
and `BOOQABLE_TIMEZONE` (naive `starts_at`/`stops_at` values from the guide
are interpreted in this timezone before converting to UTC for Booqable) —
same pattern as every other setting in this app, plain module constants read
once from `.env`. The LLM never sees `BOOQABLE_API_KEY` either way — it's
only ever read by `BooqableClient`, constructed inside this app's own
process.
...
and never uploaded
        │  only the rendered instructions go on the wire
        ▼
guideants_admin.update_guide_instructions()  -- a verified read-modify-write
        ...
        GET  /api/guides/{id}            read it back ("after")
             ...
             on failure                  ONE restore PUT of "before", then raise
        │
        ▼
live guide's instructions updated in place -- tools, context options and
knowledge files untouched; GUIDEANTS_PUB_ID unchanged, no re-publish of the
phone number or the Conversation Relay config needed
```
...
**rendered instructions**, not the bundle hash — the instructions are the
only thing a push changes, so anything that leaves them identical has
nothing to send. Most configuration fields are not template slots:
`identity.greeting`/`closing`, `terminology.*`, and the business profile's
`phone`/`website`/`timezone`/`locations` appear nowhere in
`instructions.template.md`. Knowledge edits move the content hash but not
the instructions either. Yet the greeting reaches callers only through
`published_config` (see "The draft boundary" below), which is written when a
publication row is created. So:
...
- **Same instructions, different hash or snapshot** — nothing to send
  GuideAnts, but a new `succeeded` `GuidePublication` is still recorded
  carrying the new snapshot and the new hash, and `mark_published()` still
  runs. This covers both a greeting-only edit and a knowledge-only edit.
  Without it a greeting-only publish would be swallowed: the draft flag
  would never clear and `_published_greeting()` would keep speaking the old
  greeting forever.
- **Different instructions** — the full render, push and record path.
...
### Knowledge is NOT synced by Publish

This supersedes anything elsewhere in this repo saying knowledge is pushed
on Publish. Uploading knowledge meant uploading a bundle, and uploading a
bundle is the destructive path above — worse, precisely *because* the guide
has indexed knowledge files. So Publish sends the instructions and nothing
else, and the guide's vector store is edited in the GuideAnts editor.

Because that is easy to forget at exactly the wrong moment, every
publication of a guide that has knowledge items carries
`publisher.KNOWLEDGE_NOT_SYNCED_WARNING` in its `warnings` — on the push
path and on the no-push path alike — and the console shows it. Claiming a
sync that did not happen would be a lie the next phone call exposes.

`render.knowledge_files()` still turns every publishable `KnowledgeItem`
(active, and within its effective/expiration window — see
`render.is_publishable()`) into one Markdown file under
`VectorStores/default/` in the *locally built* bundle. That is what
`GET /api/concierge/bundle` hands back for inspection or archiving, and what
`knowledge_item_count` counts. `app/knowledge_store.py`'s
`seed_default_items()` populates an empty table from
`config.DEFAULT_KNOWLEDGE_ITEMS` (the shop's original vector-store document,
one item per section) on first read, so that download and that count
describe the shop's real policy set rather than an empty one. It runs on the
publish path as well as on `GET /api/knowledge`, because an admin can reach
Publish without ever opening the console's Knowledge page. Same
...
- **Console knowledge is not synced to the guide by Publish.** This supersedes the earlier "knowledge is synced on Publish" wording here and in `frontend/TODO.md`. Publish updates the live guide's *instructions* only: pushing knowledge meant uploading a bundle to `/api/guides/import`, which deletes a guide's dependent rows one non-transactional statement at a time and then fails on a foreign key precisely when the guide has indexed knowledge files (observed 2026-09-18 — see "Why the import endpoint is not used" above). So knowledge items edited in the console are stored in SQLite, rendered into the local bundle and counted, and never reach the guide's vector store; every publication of a guide with knowledge items carries `publisher.KNOWLEDGE_NOT_SYNCED_WARNING` saying so. The knowledge base is edited in the GuideAnts editor. A safe sync would need either a fixed import endpoint or a per-file GuideAnts file API; on top of that, a `document`/`url` item's Markdown is still built from its title/category/tags/content fields only — the uploaded file or linked page is stored as a filename/address and never fetched. See `docs/superpowers/specs/2026-09-16-e6-knowledge-design.md`, "Future: syncing to the published guide".
