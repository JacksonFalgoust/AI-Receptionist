...
**If a greeting or reply comes back as Twilio's own robotic voice instead of GuideAnts'** (or, before the fix below existed, came back silent): check GuideAnts' own container logs for `tts_synthesize_failed`/`asr_transcribe_rejected` next to an unprompted `tts_model_unload_start`/`asr_model_unload_start`. GuideAnts periodically reconciles its local ASR/TTS engines and doesn't wait out an in-flight request first, so a request whose timing overlaps that cycle can have its connection dropped — worse, if the engine was busy (a longer reply), forcing it to stop can itself take several seconds, occasionally longer than this app's own turn budget. `local_audio_client.py` retries once after `GUIDEANTS_RETRY_DELAY_SECONDS` for the fast case; when GuideAnts still isn't back in time, `local_demo_api._turn_response` falls back to Twilio's built-in `<Say>` as a last resort so the caller always hears something. This is a GuideAnts-side reliability quirk (see `GuideAnts/src/server/GuideAntsApi/Services/Bootstrap/LocalAiRuntimeWatchdogHostedService.cs`), not something tunable from this app beyond the retry delay.

**If a tool-using turn (a reservation question) speaks `LOCAL_TIMEOUT_PHRASE` even though GuideAnts' own UI shows a finished text reply**: check `logs/app.log` (or GuideAnts' `webapi-ui` request log) for a `/v1/responses` call that got a 400 with `tool_results_not_pending`, immediately followed by a fresh, empty-history `/v1/responses` call. That's `guide_client.py`'s `_is_stale_tool_result` recovery (stream_reply's `_is_stale_tool_result` branch): GuideAnts cancelled the turn carrying a tool call server-side (typically from an earlier mid-stream disconnect), so this app throws away the conversation and starts completely fresh — but that retry has to fit in whatever's left of `LOCAL_TURN_BUDGET_SECONDS`, which for a turn that's already run one Booqable round-trip may not be much. GuideAnts keeps running the original (abandoned) conversation in the background regardless, which is why its own UI still shows a finished reply the caller never heard. Raising `LOCAL_TURN_BUDGET_SECONDS` gives the retry more room but can't guarantee it fits under Twilio's ~15s ceiling; there's no full fix from this app alone for a turn that needs more time than that.
...
| `LOCAL_TURN_BUDGET_SECONDS` | 11 | Deadline for the whole turn pipeline (fetch recording + STT + guide + TTS). This plus `LOCAL_FALLBACK_TTS_BUDGET_SECONDS` must stay comfortably under Twilio's ~15s webhook timeout. Reservation/tool-using turns (`checkAvailability`, `createOrder`, `sendPaymentLink` chained together) may need this raised further, since each Booqable round-trip adds real latency inside the same budget — see "If a tool-using turn times out" below for a failure mode raising this alone won't fully fix. |
